...
L.FlexGrid.prototype = {
		  ...
		  $.each(trelement, function(thindex, thelement) {
		    ...
		    if(thelement != 0) {
		      if($(thelement).attr("data-field")) {
		        ...
		        columnMap['bSortable'] = false;//默认是否排序 2017.07.03 wbw 
	...
	reload : function(url, param, resetPaging){// 重新加载数据
		...
		var oldPageInfo = _that.oTable.page.info();
		var oldPage = oldPageInfo.page;
		var oldPages = oldPageInfo.pages;
		
		_that.oTable.ajax.reload(null, true); //resetPaging true从第一页开始加载，resetPaging false 从本页加载
		

		if (resetPaging == false) {
			
			var newPageInfo = _that.oTable.page.info();
			var newPages = newPageInfo.pages;
			
			var page = oldPage + 1;

			if (oldPages == newPages) {
				_that.oTable.page(oldPage).draw(false);
			} else {
				if(page == oldPages){
					_that.oTable.page('last').draw(false);
				}else{
					_that.oTable.page(oldPage).draw(false);
				}
			}
...
function isString(value) {
	...
	if (value != '' && !regx.test(value)) {
		return {
			valid : false,
			msg : L.EditGrid.locale['msg_isString']
		};
	}
	return {
		valid : true,
