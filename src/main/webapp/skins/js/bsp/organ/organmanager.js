	...
	for (var i = 0, l = nodes.length; i < l; i++) {
		...
		for (var j = 0; j < resultArray.length; j++) {
			if (tree.getNodeByParam("struId", resultArray[j]) == nodes[i]) {
				...
				if(count==0){
					if(nodes[i].organName.indexOf($("#queryorganName").val())!=-1){
						node=nodes[i];
						count++;
					}
				}
	...
	if (resultArray.length > 0) {
		...
		if (queryNode) {
			for(var i=0;i<resultArray.length;i++){
				queryNode=tree.getNodeByParam("struId",resultArray[resultArray.length-1-i]);
...
// 选择用户
function selectUser() {
	var msg = L.getLocaleMessage("bsp.organ.071","选择用户");
	$.dialog({
		type : "iframe",
